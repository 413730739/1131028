...
function draw() {
  ...
  for (let u=0; u<points.length; u++) { 
    //text(str(i),points[i].x,points[i].y)
      noStroke()
      ellipse(points[u].x+frameCount,points[u].y,8)
      fill("#08d9d6")
      let r=map(sin(frameCount),-1,1,50,305)
      let g=map(cos(frameCount/2),-1,1,50,285)
      let b=map(sin(frameCount/4),-1,1,50,205)
      fill(r, g, b);
      angle=sin(frameCount)*15;
      //strokeWeight(5)
      //stroke("#08d9d6")
